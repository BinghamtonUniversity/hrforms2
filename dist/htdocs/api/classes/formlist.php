		...
		switch($this->req[0]) {
			...
			case "archived":
				$qry = "select f.form_id, f.created_by.SUNY_ID as created_by_suny_id, 
					...
					f.form_data.GROUPS,
					j.journal_status,
					to_char(j.max_journal_date,'DD-MON-YYYY HH24:MI:SS') as max_journal_date
				from hrforms2_forms_archive f
				join (select j.form_id, max(j.journal_date) as max_journal_date, max(j.sequence) as sequence,
				listagg(status,',') within group (order by sequence) as journal_status
				from hrforms2_forms_journal_archive j
				where (status != 'S' and group_to in (select group_id from hrforms2_user_groups where suny_id = :suny_id)) 
				or (status = 'S' and suny_id = :suny_id)
				group by j.form_id) j on (j.form_id = f.form_id)";
				break;
