		...
		switch($this->k['id']) {
			...
			case "request_id":
				...
				$qry = "select count(r.request_id)
					...
					where jr2.group_to in (select group_id from hrforms2_user_groups where suny_id = :suny_id)) j
			...
			case "form_id":
				...
				$qry = "select count(f.form_id)
					...
					where jf2.group_to in (select group_id from hrforms2_user_groups where suny_id = :suny_id)) j
