    ...
    function DELETE() {
        switch($this->req[0]) {
            case "request":
                ...
                $r = oci_execute($stmt);
                if (!$r) $this->raiseError();
                $qry = "delete from HRFORMS2_REQUESTS_JOURNAL where status in ('D','Z') and request_id = :request_id";
                $stmt = oci_parse($this->db,$qry);
                oci_bind_by_name($stmt,":request_id",$this->req[1]);
                $r = oci_execute($stmt);
                if (!$r) $this->raiseError();
            ...
            case "form":
                ...
                $r = oci_execute($stmt);
                if (!$r) $this->raiseError();
                $qry = "delete from HRFORMS2_FORMS_JOURNAL where status in ('D','Z') and form_id = :form_id";
                $stmt = oci_parse($this->db,$qry);
                oci_bind_by_name($stmt,":form_id",$this->req[1]);
                $r = oci_execute($stmt);
                if (!$r) $this->raiseError();
