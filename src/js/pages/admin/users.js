...
function UsersTable({users,newUser,setNewUser}) {
    ...
    const handleRowClick = useCallback(row=>{
        if (!row.active) {
            toast('Cannot impersonate an inactive user',{type:'error'});
            return false;
        }
        setImpersonateUser(row);
    });
        ...
        {name:'Actions',id:'actions',cell:row=>{
            return (
                <div className="button-group">
                    {row.SUNY_ID && <AppButton format="edit-user" size="sm" title="Edit User" onClick={()=>setSelectedRow(row)} disabled={row.SUNY_ID==SUNY_ID}/>}
